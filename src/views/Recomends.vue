    ...
    <div class="flex flex-wrap justify-start items-start xl:gap-2 w-full xl:w-80-vw min-h-screen text-lg bg-black p-4 lg:p-8 mt-8 mb-4 lg:mt-0">
        ...
        <div class="flex justify-center w-full relative pl-2 overflow-hidden">
            ...
            <ButtonView
                @click="scrollLeft()" 
                :tag="left" 
                class="hidden lg:flex absolute justify-center items-center w-8 h-full bgl text-white top-0 left-0 opacity-30 hover:opacity-100 z-10"/>

             <!-- caixa movel -->
            <div                 
                id="rolagem" 
                class="flex items-center gap-1 min-w-auto h-80 pt-2 pb-4 overflow-scroll overflow-y-hidden" 
         ...
         <!-- Exibição de apresentação do filme -->
        <OutDoor 
            v-if="trending && trending.length > 0"
            :movie="movie"
        />
...
export default {
    ...
    data() {
        return {
            trending : [],  // recebe os filmes em alta do vuex

            widthMax : 0,   

            movie : null,

            isScroll : false,   // manipular rolagem da nav bar fixa
    ...
    // Inicializando eventos de vigia 
    mounted() {
